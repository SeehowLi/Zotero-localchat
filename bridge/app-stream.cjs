...
class AppStream extends EventEmitter {
  ...
  async models(){await this.modelMenu();const {strength,power}=await this.command('models');await this.click('选择模型');await this.menuReady('[role=menuitemradio],[role=radio]');try{const data=await this.command('models');if(!data.models.length)throw Error('App 模型菜单未就绪');return {...data,strength,power};}finally{await this.key('Escape','Escape',27);}}
  async model(name){await this.modelMenu();await this.click('选择模型');await new Promise(r=>setTimeout(r,120));await this.cdp.evaluate('globalThis.__paperChatObserver.one('+JSON.stringify(name)+',"[role=menuitemradio],[role=radio]").click()');await new Promise(r=>setTimeout(r,150));await this.key('Escape','Escape',27);const data=await this.models();if(!data.models.some(m=>m.name===name&&m.selected))throw Error('App 尚未确认模型切换，请重连后检查');return data;}
  async strength(delta,index){await this.modelMenu();const data=await this.command('models');if(index!==undefined){if(!Number.isInteger(index)||index<0||index>data.power?.max)throw Error('Invalid strength');delta=index-data.power.index;}else if(![-1,1].includes(delta))throw Error('Invalid strength');const key=delta<0?'ArrowLeft':'ArrowRight';for(let i=0;i<Math.abs(delta);i++) await this.cdp.evaluate('globalThis.__paperChatObserver.one("强度","[role=menuitem]").dispatchEvent(new KeyboardEvent("keydown",{key:'+JSON.stringify(key)+',code:'+JSON.stringify(key)+',bubbles:true,cancelable:true}))');await new Promise(r=>setTimeout(r,120));await this.cdp.evaluate('globalThis.__paperChatObserver.one("强度","[role=menuitem]").dispatchEvent(new KeyboardEvent("keydown",{key:"Enter",code:"Enter",bubbles:true,cancelable:true}))');await new Promise(r=>setTimeout(r,120));return this.models();}
