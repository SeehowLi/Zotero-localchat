...
class Sessions {
  ...
  info(id){const p=this.context(id),b=this.bindings[p.key];return{draftKey:p.key,mode:p.mode,title:p.title,excerpt:p.excerpt||'',binding:b?.title||null,uploaded:!!b?.uploaded,reused:!!b?.reused,...(b?.phase==='failed'?{pendingDraft:b.pendingPrompt||'',failure:b.failure||''}:{}),phase:b?.phase||'idle'};}
