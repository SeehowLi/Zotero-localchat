...
class Sessions {
  ...
  info(id){const p=this.context(id),b=this.bindings[p.key];return{draftKey:p.key,mode:p.mode,title:p.title,excerpt:p.excerpt||'',binding:b?.title||null,uploaded:!!b?.uploaded,phase:b?.phase||'idle'};}
