...
class Setup {
  ...
  async status(){
    ...
    return {...state,project,ready:state.debugReady&&exists&&!!(this.project||legacy),legacy};
  }
  async select(project){
    ...
    const state=await this.host.inspect();if(!state.debugReady)throw Error('请先使用 ChatGPT - Local Chat 快捷方式打开 App');
