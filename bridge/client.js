...
function controls(){
  ...
  $('model').disabled=busy||!ready;$('power').disabled=busy||!ready;
...
function models(data,preserve=false){
  ...
  if($('model').dataset.catalog!==signature){$('model').replaceChildren(...data.models.map(m=>{const o=document.createElement('option');o.value=m.name;o.textContent=m.name;o.disabled=!m.enabled;return o;}));$('model').dataset.catalog=signature;}
  $('model').value=preserve&&configPending.model?configPending.model:data.models.find(m=>m.selected)?.name||'';
  power=data.power;$('powerLabel').hidden=!power;
  if(power){
...
function powerText(value,fallback){const index=Math.round(value),label=power?.options?.[index]?.label;const text=label||fallback?.split(/[，,]/)[0]||`档位 ${index+1}`;if($('powerText').textContent!==text){$('powerText').textContent=text;$('power').setAttribute('aria-valuetext',text);}$('power').style.setProperty('--progress',`${power?.max?value/power.max*100:0}%`);}
function scheduleConfig(){clearTimeout(configTimer);configTimer=setTimeout(()=>{configTimer=0;flushConfig();},180);controls();}
...
function commitPower(){if(busy||!ready||!power)return;previewPower=Math.round(Number($('power').value));$('power').value=previewPower;powerText(previewPower);configPending.index=previewPower;scheduleConfig();}
...
$('power').onkeydown=e=>{if(!power||!['ArrowLeft','ArrowDown','ArrowRight','ArrowUp','Home','End','PageUp','PageDown'].includes(e.key))return;e.preventDefault();const down=['ArrowLeft','ArrowDown','PageDown'].includes(e.key),next=e.key==='Home'?0:e.key==='End'?power.max:Math.round(Number($('power').value))+(down?-1:1);$('power').value=Math.max(0,Math.min(power.max,next));commitPower();};
$('power').addEventListener('wheel',e=>{if(e.ctrlKey)return;e.preventDefault();if(busy||!ready||!power||dragging)return;const next=Math.min(power.max,Math.max(0,Math.round(Number($('power').value))+(e.deltaY<0?1:-1)));if(next===Number($('power').value))return;$('power').value=next;commitPower();},{passive:false});
