...
$xpi=Join-Path $dist 'Zotero-localchat-0.4.13.xpi'
...
$zip=Join-Path $dist 'Zotero-localchat-0.4.13-windows.zip'
...
try {
  ...
  foreach($name in @('README.md','CHANGELOG.md','CONTRIBUTORS.md','LICENSE','Start-LocalChat.cmd','Stop-LocalChat.cmd')){[IO.Compression.ZipFileExtensions]::CreateEntryFromFile($archive,(Join-Path $root $name),$name) | Out-Null}
  [IO.Compression.ZipFileExtensions]::CreateEntryFromFile($archive,$xpi,'Zotero-localchat-0.4.13.xpi') | Out-Null
