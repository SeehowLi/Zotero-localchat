...
test('real CDP: DOM events, background input, model menu, and no duplicate sends',async t=>{
  ...
  let uiPhase='ready',recoverCalls=0;
  let setupReady=true,setupProject='Paper',openAppCalls=0,sessionOpens=0;
  ...
  await app.cdp.evaluate(`document.querySelector('main').insertAdjacentHTML('beforeend','<aside id="network-error" role="alert">net::ERR_CONNECTION_CLOSED<button>重试</button></aside>')`);
  assert.equal((await app.read()).responseError,'net::ERR_CONNECTION_CLOSED');
  await app.cdp.evaluate(`document.getElementById('network-error').remove()`);assert.equal((await app.read()).responseError,null);
  ...
  uiPhase='uncertain';const beforeNetworkRetry=uiSends;
  await app.cdp.evaluate('document.getElementById("reconnect").click()');await until(()=>app.cdp.evaluate('ready&&!busy'));
  assert.equal(recoverCalls,1);assert.equal(uiSends,beforeNetworkRetry);assert.equal(await app.cdp.evaluate('document.getElementById("prompt").value'),'new draft','network recovery preserves newer edits');
  await app.cdp.evaluate('localStorage.clear()');await app.cdp.call('Page.reload');await until(()=>app.cdp.evaluate('typeof ready!=="undefined"&&ready&&!busy'));
  assert.equal(await app.cdp.evaluate('document.getElementById("prompt").value'),'network question','server keeps the failed question even when browser storage is lost');assert.equal(uiSends,beforeNetworkRetry);
  uiPhase='ready';
  // First-run setup stays local until the explicit project choice.
  setupReady=false;setupProject='zotero-paper';const opensBefore=sessionOpens,sendsBefore=uiSends;
  await app.cdp.call('Page.reload');await until(()=>app.cdp.evaluate('!!document.getElementById("setup")&&!document.getElementById("setup").hidden&&!busy'));
  assert.equal(sessionOpens,opensBefore);assert.equal(uiSends,sendsBefore);assert.equal(await app.cdp.evaluate('document.getElementById("composer").hidden'),true);assert.equal(await app.cdp.evaluate('document.getElementById("setupProject").value'),'zotero-paper');
  await app.cdp.evaluate('document.getElementById("setupOpen").click()');await until(()=>app.cdp.evaluate('!setupBusy'));assert.equal(openAppCalls,1);
  await app.cdp.evaluate('document.getElementById("setupProject").value="My research";document.getElementById("setupCheck").click()');await until(()=>app.cdp.evaluate('!setupBusy&&!busy'));assert.equal(await app.cdp.evaluate('document.getElementById("setupProject").value'),'My research','checking preserves the entered name');
  assert.equal(await app.cdp.evaluate('document.documentElement.scrollWidth<=innerWidth'),true,'setup fits the narrow sidebar');
  await app.cdp.evaluate('document.getElementById("setupUse").click()');await until(()=>app.cdp.evaluate('!setupBusy&&ready&&!busy'));assert.equal(setupProject,'My research');assert.equal(await app.cdp.evaluate('document.getElementById("setup").hidden'),true);assert.match(await app.cdp.evaluate('document.getElementById("paper").textContent'),/My research/);assert.equal(uiSends,sendsBefore,'finishing setup never sends a message');
});
