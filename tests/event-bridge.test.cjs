...
test('real CDP: DOM events, background input, model menu, and no duplicate sends',async t=>{
  ...
  if(process.env.LOCALCHAT_SCREENSHOT){await app.cdp.call('Emulation.setEmulatedMedia',{features:[{name:'prefers-color-scheme',value:'dark'}]});fs.writeFileSync(process.env.LOCALCHAT_SCREENSHOT,Buffer.from((await app.cdp.call('Page.captureScreenshot',{format:'png'})).data,'base64'));}
